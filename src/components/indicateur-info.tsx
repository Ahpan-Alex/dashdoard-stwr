...
import {
  ...
  type IdIndicateur,
} from "@/lib/referentiel-indicateurs";
...
export function IndicateurInfo({
  ...
  const def = indicateur ? definitionIndicateur(indicateur) : undefined;
  const titreFinal = titre ?? def?.titre ?? "D’où vient ce chiffre ?";
  return (
    <span className="relative inline-flex align-middle">
      ...
      {open && (
        ...
        >
          {def ? <span className="block">{def.definition}</span> : null}
          {children ? (
            <span className={def ? "mt-2 block" : "block"}>{children}</span>
          ) : null}
        </span>
      )}
    </span>
