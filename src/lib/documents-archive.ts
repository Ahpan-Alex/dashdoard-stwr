...
export const TYPES_ARCHIVE_FOURNISSEUR: TypeDocumentArchive[] = [
  "demande_prix",
  "commande_fournisseur",
];
...
export type FamilleArchiveDocuments = "fournisseur" | "vente";
...
export function collecterDocumentsArchive(opts: {
  ...
  const types = famille === "fournisseur" ? TYPES_ARCHIVE_FOURNISSEUR : TYPES_ARCHIVE_VENTE;
  const garder = (t: TypeDocumentArchive) => types.includes(t);

  if (garder("demande_prix")) {
    for (const d of demandesPrix ?? []) {
      const ids = d.fournisseurIdsRetenus ?? d.fournisseurIds ?? [];
      rows.push({
        id: d.id,
        type: "demande_prix",
        numero: d.numero,
        date: d.date,
        tiersIds: ids,
        tiersNom: ids.map((id) => nom(id)).filter((n) => n !== "—").join(", ") || "—",
        statut: d.statut,
  ...
  if (garder("commande_fournisseur")) {
